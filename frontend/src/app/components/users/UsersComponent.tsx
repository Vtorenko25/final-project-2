...
//                 let fetchedUsers: IUser[] = Array.isArray(data.data) ? data.data : data;
//
...
//     }, [debouncedFilters, page, sortColumn, sortOrder]);
//
...
//                     <input type="checkbox"/> My
//                 </label>
//                 <button className="reset-button" title="Скинути фільтри" onClick={resetFilters}>⟳</button>
//                 <button className="excel-button" title="Експорт в Excel">📄</button>
//             </div>
//
//             <HeaderComponent
...
//                             <li>{user.created_at ? new Date(user.created_at).toLocaleDateString("en-US", {
...
export default function UsersComponent() {
    ...
    const handleAddComment = useCallback(async (user: IUser) => {
        ...
        if (!text) return;
        try {
            const dto: IComment = { userId: user._id, crmId: user.id, content: text, manager: user.manager || "admin", createdAt: new Date().toISOString(), title: "" };
            const savedComment = await commentService.createComment(dto);
            setComments(prev => ({ ...prev, [user._id]: [...(prev[user._id] || []), savedComment] }));
            const inWork = await userService.updateUserById(user._id, { status: "In Work", manager: "admin" });
            setUsers(prev => prev.map(u => (u._id === user._id ? { ...u, ...inWork } : u)));
            handleInputChange(user._id, "");
        } catch (err) { console.error("Помилка при створенні коментаря:", err); }
    }, [newComment]);
