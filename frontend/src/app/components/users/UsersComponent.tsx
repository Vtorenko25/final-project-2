...
//
//     const handleSortChange = (column: string, order?: "asc" | "desc") => {
//         const col = column as keyof IUser;  // кастимо string в keyof IUser
...
//
//         router.replace(`${window.location.pathname}?${params.toString()}`, { scroll: false });
//     }, [debouncedFilters, page, sortColumn, sortOrder, router]);
//
//     const handleFilterChange = (field: string, value: string) => {
//         setFilters(prev => ({ ...prev, [field]: value }));
//     };
...
//         setDebouncedFilters(defaultFilters);
//         const params = new URLSearchParams();
//         params.set("page", page.toString()); // залишаємо поточну сторінку
//         if (sortColumn) params.set("order", sortColumn);
//         if (sortOrder) params.set("direction", sortOrder);
//         router.replace(`${window.location.pathname}?${params.toString()}`, { scroll: false });
//         setMyOnly(false);
...
export default function UsersComponent({ setTotalUsers, usersPerPage }: UsersComponentProps) {
    ...
    // Читаємо сторінку з URL при першому рендері
    const [page, setPage] = useState<number>(() => parseInt(searchParams.get("page") || "1", 10));

    const displayValue = (value: any) =>
        value === null || value === undefined || value === "" ? "null" : value;

    // Ініціалізація фільтрів та сортування з URL
    useEffect(() => {
        const newFilters = { ...defaultFilters };
        Object.keys(defaultFilters).forEach(key => {
            const val = searchParams.get(key);
    ...
    // **Синхронізація URL при зміні фільтрів, сортування чи сторінки**
    useEffect(() => {
        if (isFirstRender.current) {
            isFirstRender.current = false;
            return;
    ...
    const handleFilterChange = (field: string, value: string) => {
        setFilters(prev => ({ ...prev, [field]: value }));
        setPage(1); // **скидаємо сторінку на 1 при зміні фільтрів**
    };
    ...
    const handleSortChange = (column: string, order?: "asc" | "desc") => {
        ...
        setSortColumn(col);
        setSortOrder(newOrder);
        // При сортуванні залишаємо поточну сторінку
    };
